			...
			if (data.status == 200) {
				...
				$.each(data.maps, function(index, value) {
					...
					tableHtml += `<tr class="text-center `+rowClass+`">
						...
						<td><p class="text-center"><input name="beatmaps[`+escapeHtml(String(value.id))+`]`+escapeHtml(String(value.id))+`" value="loved" type="radio"></p></td>
				...
				tableHtml += `<button id="loved-all" type="button" class="btn btn-direct"><span class="fa fa-heart"></span>	Loved everything</button>`;
				...
				tableHtml += `<a href="https://vipsu.ml/d/`+escapeHtml(String(bsid))+`" target="_blank" type="button" class="btn btn-info"><span class="glyphicon glyphicon-arrow-down"></span>	Download beatmap set</a>`;
...
function updateTriggers() {
	...
	$("#loved-all").click(function() {
		$("[value=loved]").prop("checked", true);
	});
