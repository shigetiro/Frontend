...
class P {
	...
	*/
	...
	public static function AdminDashboard() {
		...
		//$totalPPQuery = 0;//$GLOBALS['db']->fetch("SELECT SUM(pp) FROM scores WHERE completed = 3 LIMIT 1");
		$totalPP = 0;
	...
	*/	 
		...
		/*foreach ($recentPlays as $play) {
			...
			echo '<td><p class="text-left"><a href="https://new.vipsu.ml/b/'.$play["beatmap_id"].'">'.$bn.'</a> <b>' . getScoreMods($play['mods']) . '</b></p></td>';
	...
	*/
	public static function AdminEditUser() {
		try {
			...
			$ips = $GLOBALS['db']->fetchAll('SELECT any_value(id)id, any_value(ip)ip FROM ip_user WHERE userid = ? GROUP BY ip ORDER BY id DESC LIMIT 10', $_GET['id']);
			...
			if ($userData['username'] == $_SESSION['username']) {
				...
				redirect("/p/102&e=You don't have enough permissions to edit this user chree");
				die();
			}
			...
			echo '<tr>
				<td>VK</td>
				<td><a href="https://vk.com/id'.$vk.'">https://vk.com/id'.$vk.'</a></td>
				</tr>';
			...
			<td>
				<p align="center">
					<img src="'.URL::Avatar().'/'.$_GET['id'].'" height="64" width="64" style="width: 64px;" ></img>
				</p>
			</td>
			</tr>';
			...
			foreach ($ips as $ip) {
				$ip = explode(", ", $ip["ip"])[0];
				echo "<li>$ip <a class='getcountry' data-ip='$ip' title='Click to retrieve IP country'>(?)</a></li>";
			}
			echo '</ul></td></tr>';
			if($userData["allowed"] == 0){
				...
				<td>Trace</td>
				<td>
						<a href="http://vipsu.ml/web/osu-gettrace.php?k='.md5($letsKey).'&c='.$userData["id"].'"><span class="label label-danger">Download trace</span></a>
				</td>
				</tr><br>';
			}
			echo '</tbody></form>';
			...
			echo '<div class="text-center" style="width:50%; margin-left:25%;">
					...
					if (hasPrivilege(Privileges::AdminBanUsers)) {
						...
						echo '<br><br><a onclick="sure(\'/submit.php?action=traceUser&id='.$_GET['id'].'\');" class="btn btn-danger">'.(($userData["allowed"] == 1) ? "Start" : "Stop").' trace user</a>';
					}
	...
	*/
	...
	if($count < 175)
		...
		if ($percText < 10) {
			...
			$text = "175 Дружбанов для введения системы кланов";
        echo '<div class="col-lg-12 text-center">
        <p align="center"><img src="/images/logos/logo.png" style="width:50%"></p>
        <div class="col-lg-12 text-center"><div class="col-lg-3 text-center"></div><div class="col-sm-6"><b>'.$count.'/'.$text.'</b>
<div class="progress">
<div class="progress-bar" role="progressbar" aria-valuenow="'.$percBar.'" aria-valuemin="10" aria-valuemax="100" style="width:'.$percBar.'%">'.bound($percText, 1, 100).'%</div></div>
    </div></div>
<div class="col-lg-3 text-center"><div class="panel-heading"><h5><b> Top supporters
	...
	';
		...
		$druzhBans = $GLOBALS["db"]->fetchAll("SELECT id,username,donor_expire FROM users WHERE id NOT IN (1000,999,1167) ORDER BY `donor_expire` DESC LIMIT 5");
	
$top = 0;
		foreach($druzhBans as $druzhBan)
{ 
$top++;
switch ($top) {
	case 1:
	...
	$expireString = trim(timeDifference(time(), $druzhBan["donor_expire"], false,"", True));
    echo'
<tr>
<td class="warning">
<div class="user-home__beatmapsets">
<a class="user-home-beatmapset" href="https://new.vipsu.ml/u/'.$druzhBan['id'].'">
<img class="user-home-beatmapset__cover" src="https://a.vipsu.ml/'.$druzhBan['id'].'">
<div class="user-home-beatmapset__meta"><div class="user-home-beatmapset__title u-ellipsis-overflow">'.$druzhBan['username'].'</div>
<div class="user-home-beatmapset__creator u-ellipsis-overflow">'.$expireString.'</div> 
</div><div class="punch"><span class="punch-'.$punch.'"></span></div></a>
...
<td class = "warning">
<div class = "user-home__beatmapsets">
<a class = "user-home-beatmapset" href = "https://vipsu.ml/s/'.$play["beatmapset_id"].'">
...
<iframe src="https://discordapp.com/widget?id=325414419041615872&theme=dark" width="350" height="500" allowtransparency="true" frameborder="0"></iframe>
...
echo '
		...
		</div><a href="//www.free-kassa.ru/"><img src="//www.free-kassa.ru/img/fk_btn/21.png"></a>';
		...
		public static function Match($matchID)
{
		...
		echo '<br><hr><div id="matchinfo"></div><div> ';
$merchant_id = '55473';
$secret_word = 'w1863dxt';
$order_id = '3';
$order_amount = '1';
$sign = md5($merchant_id.':'.$order_amount.':'.$secret_word.':'.$order_id);
echo "

<a href='http://www.free-kassa.ru/merchant/cash.php?m=55473&oa=".$order_amount."&o=".$order_id."&s=".$sign."&lang=ru&i=&em='>click</a>
";
		echo'</div><br><hr>';
}
	public static function Beatmap($beatmap, $type)
	{
		...
		echo(htmlspecialchars($song_name));
		echo("</b></h2>");
		if($beatmapData["creator"] != ''){
			echo("<h4>Created by ".$beatmapData["creator"]."</h4>");
		}echo '</br>';
        ...
        preg_match_all($re, $str, $matches);
        $diff = $matches[2][0];
		echo '<li><a class="beatmapTab '.($beatmap["beatmap_id"] == $beatmapData["beatmap_id"] ? "selected":"").'" href="/b/'.$beatmap["beatmap_id"].'"><div class="diffIcon '.$diffName.'"></div><span>'.htmlspecialchars($diff).'</span></a></li>';
...
echo'</div>';
checkLoggedIn();
$druzhBan = hasPrivilege(Privileges::UserDonor,$_SESSION["userid"]);
...
}else echo '<div class="beatmap-downlaod"><a class="btn btn-blue" id ="favourite" href="/p/2"> Add to favourites</a></div>';
  echo '<div class="beatmap-downlaod"><a href="https://vipsu.ml/d/'.$beatmapData["beatmapset_id"].'"><button class="btn btn-default" '.($druzhBan ? "" : "style='padding: 6px 31px;'").' >Download Beatmap</button></a></div>';
  echo'<div class="beatmap-downlaod"><a href="osu://dl/'.$beatmapData["beatmapset_id"].'"><button class="btn btn-direct">Open in osu!Direct</button></a></div>'; 
  if ($druzhBan)
  echo'<div class="beatmap-ezpp"><a href="#" class="ezpp show" data-toggle="modal" data-target="#ezpp" title=""></a></div>';
echo'</div>';
...
<a class="btn mode" id="easy"></a></div>';
		...
		<thead>
		<tr><th class="text-left"> Rank</th><th>Player</th>'.(($beatmapData["mode"] == 0 || $beatmapData["mode"] == 3 || $beatmapData["mode"] == 1) ? '<th>PP</th>' : '').'<th>Score</th><th>Accuracy</th><th>Max Combo</th><th>300 / 100 / 50</th><th>Misses</th><th>Mods</th><th class="text-right">Date</th>'.($_SESSION["userid"] != 0 ? '<th> </th>' : '').'</tr>
		</thead>';
			// Print row
...
echo '<hr>';
	...
	if ($druzhBan){
	echo '<div id="scriptgovna"><script type="text/javascript">var sc = document.getElementById("scriptgovna");
	var content = document.getElementsByClassName("content")[0];
	var song_name = "'.$beatmapData["artist"].' - '.$beatmapData["title"].'";
	var newElement = document.createElement(\'div\');
	newElement.innerHTML = \'<div class="modal fade" id="ezpp" tabindex="-1" role="dialog"\ aria-labelledby="exampleModalLabel" aria-hidden="true">\
  <div class="modal-dialog" role="document">\
    <div class="modal-content">\
      <div class="modal-body"><div class="ezpp cardheader" id="'.$beatmapData["beatmapset_id"].'" style="background: url(https://assets.ppy.sh/beatmaps/'.$beatmapData["beatmapset_id"].'/covers/cover.jpg);"><span class="ezpp songname">\'+song_name+\'</span></div><div class="alert alert-warning" role="alert"><div class="input-group" style="width:100%"><div class="col-lg-4"><span class="ezpp placeholder">Accuracy</span><input pattern="\d+(\.\d{2})?" type="text" value="100" id="ezpp-accuracy" class="ezpp form-control-black" aria-describedby="basic-addon3" spellcheck="false"></div><div class="col-lg-4"><span class="ezpp placeholder">Combo</span> <span class="ezpp hint">(empty = FC)</span><input type="text" pattern="^[ 0-9]+$" value="'.$beatmapData["max_combo"].'" id="ezpp-combo" class="ezpp form-control-black" aria-describedby="basic-addon3" spellcheck="false"></div><div class="col-lg-4"><span class="ezpp placeholder">Misses</span><input type="text" pattern="^[ 0-9]+$" value="0" class="ezpp form-control-black" id="ezpp-misses" aria-describedby="basic-addon3" spellcheck="false"></div></div></div><div class="ezpp alert alert-up" role="alert"><div class="ezpp mods"><a class="ezpp btn mode" id="hidden"><span class="ezpp mod">Hidden</span></a> <a class="ezpp btn mode" id="hardrock"><span class="ezpp mod">Hard Rock</span></a> <a class="ezpp btn mode" id="double-time"><span class="ezpp mod">Double Time</span></a><a class="ezpp btn mode" id="flashlight"><span class="ezpp mod">Flashlight\
    </span></a><a class="ezpp btn mode" id="easy"><span class="ezpp mod">Easy</span></a>\
	<a class="ezpp btn mode" id="half-time"><span class="ezpp mod">Half Time</span></a></div></div>\
     <div class="alert alert-warning" role="alert">\
<div class="result-wrapper">\
    <div class="ezpp result" id="result">That\\\'s about 166pp.</div>\
  </div>\
</div>\
      </div>\
      <div class="modal-footer">\
	...
	*/
	...
	public static function ChartPage(){
		...
		try {
			$counts = [];
			$expireString = trim(timeDifference(time(), 1502053200, false));	
			if(1502053200 - time() < 0){
				echo '<h1><i class="fa fa-clock-o animated infinite pulse"></i>	Expect a new chart
!</h1>';	
				return;
			}

		echo '<h1><i class="fa fa-clock-o animated infinite pulse"></i>	To the end left: '.$expireString.'</h1>';	
		echo'<br>
		...
		echo '<tbody>';
			...
			foreach($data as $key => $e){
				...
				array_push($counts[$e["beatmap_md5"]],["userid"=>$e["userid"], "score"=>(floatval($e["pp"])),"difficulty"=>floatval($e["difficulty_std"])]);
			}
			$price = array();
	...
	public static function ClanPage($id){
		global $ScoresConfig;
		global $PlayStyleEnum;
		...
		try {
			...
			$u = safeUsername(htmlspecialchars($u));
			

			// Check banned status
			$clanData = $GLOBALS['db']->fetch("SELECT * FROM clans WHERE id = ?", [$id]);
			if (!$clanData) {
				// LISCIAMI LE MELE SUDICIO
				throw new Exception('Clan '.$id.' not found');
			}
			...
			echo '<title> '.$clanName."</title>";

            $userpageContent = $clanData["userpage_content"];
	...
	$userData['user_color'] = str_replace("#000000","white",$userData['user_color']);
			...
			echo '<div class="col-sm-2" id="prof-left"><div class="alert alert-up" role="alert"><div class="avatar"><img src="https://a.vipsu.ml/'.$userID.'"></div>';
			echo'<div style="font-size: 105%; '.$userStyle.'"><b style="'. (!empty($userData['user_color']) ? "color: ".str_replace("white","#333",$userData[user_color])." !important;" : "").' " title="'.htmlspecialchars($userData["username_aka"]).'">	'.$clanName.'</b></div>';
			...
			if (!checkLoggedIn() || $username == $_SESSION['username']) {
                ...
                switch ($friendship) {
					case 1:
						$friendButton = '<div id="friend-button"><a onclick="addRemoveFriend(this);" userid="'.$u.'" type="button" class="btn btn-success"><span class="glyphicon glyphicon-star"></span>	Friend</a></div>';
					break;
					case 2:
						$friendButton = '<div id="friend-button"><a onclick="addRemoveFriend(this);" userid="'.$u.'" type="button" class="btn btn-danger"><span class="glyphicon glyphicon-heart"></span>	Mutual Friend</a></div>';
					break;
					default:
						$friendButton = '<div id="friend-button"><a onclick="addRemoveFriend(this);" userid="'.$u.'" type="button" class="btn btn-primary"><span class="glyphicon glyphicon-plus"></span>	Add as Friend</a></div>';
					break;
				}
			}
echo $friendButton;
	...
	if ($friendButton == '') $margin = 8; else $margin = -8;
			...
			if (hasPrivilege(Privileges::AdminManageUsers)) {
				$buttons =  $buttons.(strlen($buttons) > 0 ? "<br>" : "").'<a href="/p/103&id='.$u.'">Edit User</a>';
			}
...
echo '</div> <div class="col-sm-10 tab-content">';
			...
			//	if (strlen($userpageContent) <= 600) {
				...
				echo '<div class="spoiler">
						...
						if (checkLoggedIn() && $username == $_SESSION['username']) {
						echo '<div class="up panel-heading">';
								//echo '<button type="button" class="btn btn-default btn-xs spoiler-trigger" data-toggle="collapse">Expand userpage</button>';
				
					echo '	<a href="/p/8" type="button" class="btn btn-default btn-xs">	Edit userpage 	</a>';
				echo '</div>';
				}
				echo'
							<div class="panel-collapse collapse '.$ct.'">
								<div class="panel-body userpage-content" >'.bbcode::toHtml($userpageContent, true).'</div>
							</div>
						</div>
					</div>';
			} 
			...
			if (true == false) // ?????????????? LUL
				echo '<div class="panel-heading text-left"><h4>Performance: <b>' . number_format($pp) .' points</b> (<b>#'.$rank.'</b>)</h4></div>';
			...
			<table>
			<tbody>';
			if(hasPrivilege(Privileges::UserDonor,$_SESSION["userid"])){
				$playC = $GLOBALS["db"]->fetch("SELECT COUNT(*) as hits FROM `scores` WHERE userid = ? AND play_mode = ?",[$u, $m])["hits"];
				$hits = $GLOBALS["db"]->fetch("SELECT SUM(100_count+300_count+50_count) as hits FROM `scores` WHERE userid = ? AND play_mode = ?",[$u, $m])["hits"];
				$hitsPlay = $hits / $playC;
			echo '<tr>
			<td id="stats-name">Hits/play</td>
			<td id="stats-value"><b>'.accuracy($hitsPlay).'</b></td>
			</tr>';
			}
			echo'
			<tr>
			...
			if ($ScoresConfig["enablePP"] && ($m == 0 || $m == 3 || $m == 1))
				$scoringName = "PP";
			else
				$scoringName = "Score";
			...
			<div class="alert alert-up" role="alert"><table class="table">
			<tbody>';
					$clanUsers = $GLOBALS["db"]->fetchAll("SELECT clan_users.*,users_stats.pp_std,users_stats.username, users_stats.avg_accuracy_std FROM `clan_users` LEFT JOIN users_stats ON (users_stats.id = clan_users.userid) WHERE clanid = ?",[$clanid]);
	

		foreach($clanUsers as $user)
{ 	
	if($clanData["creator"] == $user["userid"])
		$user["username"] = "[CL] ".$user["username"];
    echo'
<tr>
<td class="warning">
<div class="user-home__beatmapsets">
<a class="user-home-beatmapset" href="https://vipsu.ml/u/'.$user["userid"].'">
<img class="user-home-beatmapset__cover" src="https://a.vipsu.ml/'.$user["userid"].'">
<div class="user-home-beatmapset__meta"><div class="user-home-beatmapset__title u-ellipsis-overflow">'.$user["username"].'</div>
<div class="user-home-beatmapset__artist u-ellipsis-overflow">'.$user["pp_std"].'pp</div>
<div class="user-home-beatmapset__creator u-ellipsis-overflow">Acc: '.round($user["avg_accuracy_std"],2).'%</div> 
</td>
...
SELECT
	users_stats.*, users.privileges,users.username,users.username_safe, users.id as usersuid, users.latest_activity,
	users.silence_end, users.silence_reason, users.register_datetime
FROM users_stats
LEFT JOIN users ON users.id=users_stats.id
WHERE  users.username_safe like ? or users.id = ? LIMIT 1", [$u,$u]);
			if (!$userData) {
				// LISCIAMI LE MELE SUDICIO
	...
	$userData['user_color'] = str_replace("#000000","white",$userData['user_color']);
			...
			if (isOnline($userData["id"])) 
				echo '<i class="fa fa-circle online-circle"></i>';
						...
						$u = $userData["id"];
			if ($u == 1000 || $u == 1015) echo '<br><div class="dev"></div>';
			if($u == 1206)
...
<div><a class="profileGameModeButton'.($m == 0 ? " active" : "").'" href="?m=0">osu!</a><a class="profileGameModeButton'.($m == 1 ? " active" : "").'" href="?m=1">Taiko</a><a class="profileGameModeButton'.($m == 2 ? " active" : "").'" href="?m=2">CatchTheBeat</a><a class="profileGameModeButton'.($m == 3 ? " active" : "").'" href="?m=3">osu!mania</a></div>
				...
				if(($m == 0 || $m == 3 )&& $pp > 0 ){ 
					...
					$history = $GLOBALS["db"]->fetchAll("SELECT * FROM user_charts WHERE userid = ? ORDER BY id DESC LIMIT 29",[$u]);
...
<td width="60"><img height="36" src="/images/ranks/a_big.png"></td><td width="60">'.$a.'</td>
			...
			foreach($logArray as $key => $log){
				...
				<p class="text-left"><a  href="https://vipsu.ml/u/'.$userID.'">'.$username.'</a> 
				'.$log["log"]. (isset($log["song_name"]) ? "<a href='https://vipsu.ml/b/".$log["beatmap_id"]."'>".$log["song_name"].'</a>' : "").'	</p></td><td class="warning"><p class="text-right"><small>'.timeSince($log['time']).' ago</small></p></td></tr>';
			}
		}
			echo '</tbody></table>';
...
echo '<div class="tab-pane" id="recent">';
			...
			<tr><th class="text-left"><i class="fa fa-clock-o"></i>	Recent plays</th><th class="text-right">' . $scoringName . '</th></tr> <tr><th class="text-left">	<input type="checkbox" id="checkbox" onclick="ShowFailed();">	<label for="checkbox">	Show failed plays</label></th><th class="text-right"></th></tr>';
			echo '</table>';
...
WHERE users.$kind LIMIT 1", [$u]);
			...
			if (hasPrivilege(Privileges::AdminManageUsers)) {
				$buttons =  $buttons.(strlen($buttons) > 0 ? " | " : "").'<a href="/p/103&id='.$u.'">Edit user</a> | <a href="/p/110&id='.$u.'">Edit badges</a>';
			}
			if (hasPrivilege(Privileges::AdminBanUsers) && $username != $_SESSION['username']) {
			...
			if ($ScoresConfig["enablePP"] && ($m == 0 || $m == 3 || $m == 1)) echo '<b>' . number_format($pp) . ' pp</b>';
			echo $friendButton;
			echo '</div>';
			echo '</div>';
...
// Calculate required score for our level
			...
			foreach($logArray as $key => $log){
				echo '<tr><td class="warning">
				<p class="text-left"><b><a style="color: #b3b3b3 !important; " href="https://new.vipsu.ml/u/'.$userID.'">'.$username.'</a> 
				</b>'.$log["log"]. (isset($log["song_name"]) ? "<a href='https://new.vipsu.ml/b/".$log["beatmap_id"]."'>".$log["song_name"].'</a>' : "").'	</p></td><td class="warning"><p class="text-right"><small>'.timeSince($log['time']).' ago</small></p></td></tr>';
			}
		}
			echo '</tbody></table>';
			echo '<table class="table" id="best-plays-table">
	...
	*/
	public static function ExceptionMessage($e, $ret = false) {
		$p = '<div class="container panel panel-red animated shake">
<div class="row" style="">

<div class="panel-heading main">
<div class="row">

    <div class="col-xs-12"> <p align="center"><b>An error occurred:<br></b>'.$e.'</p></div></div></div></div></div>';
		if ($ret) {
			return $p;
		}
		echo $p;
	...
	*/
	public static function SuccessMessage($s, $ret = false) {
				$p = '<div class="panel panel-green animated fadeIn">
<div class="row" style="">

<div class="panel-heading main">
<div class="row">

    <div class="col-xs-12"> <p align="center"><b>Success!<br></b>'.$s.'</p></div></div></div></div></div>';
		if ($ret) {
			return $p;
		}
		echo $p;
	...
	*/
	public static function userSettingsPage() {
		...
		echo '<h3>VK account</h3>

		<div class="row">
<div class="co-md-6">
';
$vk = current($GLOBALS["db"]->fetch("SELECT vk FROM users WHERE id = ?",[$data["id"]]));
if($vk == 0)

echo'<div class="alert alert-warning"><div class="ui grid dick" ><div class="ui mutual column" style="
    width: 15em;
">		<div class="content dick">
	<img class="vk-avatar" src="https://vipsu.ml/images/default-avatar.jpg"></div>
	<div class="content dick2">
			<a href="#">Guest<br>User</a><div class="sub header">				
			<a href="https://oauth.vk.com/authorize?client_id=6219459&display=popup&redirect_uri=http://vipsu.ml/vk.php&scope=offline&response_type=code&v=5.68" type="button" class="btn btn-primary">	Link account</a>';
else{
	$vkData =  getJsonCurl("https://api.vk.com/method/users.get?user_id=".$vk."&fields=photo_100")["response"][0];
	echo'<div class="alert alert-warning"><div class="ui grid dick" ><div class="ui column dick">		<div class="content dick">
	<img class="vk-avatar" src="'.$vkData["photo_100"].'"></div>
	<div class="content dick2">
			<a href="https://vk.com/id'.$vk.'">'.$vkData["first_name"].'<br>'.$vkData["last_name"].'</a><div class="sub header">				
			<a onclick="UnlinkVK();" type="button" class="btn btn-danger">	Unlink account</a>';

			echo '<script>function UnlinkVK() {
			$.getJSON("/vk.php", {
				unlink: 1,
			}, function(data) {			
				location.reload();
...
echo'			
	...
	*/
	public static function UserpageEditorPage() {
		...
		// Print Exception if set
		$exceptions = ['Nice troll.', "Your userpage <b>can't be longer than 6500 characters</b> (bb code syntax included)", "You can't edit your userpage while you're restricted."];
		if (isset($_GET['e']) && isset($exceptions[$_GET['e']])) {
			self::ExceptionMessage($exceptions[$_GET['e']]);
		}
		// Print Success if set
		...
		// Print default message if we have no exception/success
		if (!isset($_GET['e']) && !isset($_GET['s'])) {
			echo '<p>Introduce yourself here! <i>(max 6500 chars)</i></p>';
		}
		// Print form
		echo '<form action="/submit.php" method="POST">
	...
	*/
	public static function PasswordRecovery() {
		...
		$exceptions = ['Nice troll.', "That user doesn't exist.", "You are banned from Vipsu. We won't let you come back in."];
		if (isset($_GET['e']) && isset($exceptions[$_GET['e']])) {
			self::ExceptionMessage($exceptions[$_GET['e']]);
		}
		if (isset($_GET['s'])) {
			self::SuccessMessage('You should have received an email containing instructions on how to recover your Vipsu account');
		}
		if (checkLoggedIn()) {
			echo 'What are you doing here? You\'re already logged in, you moron!<br>';
			echo 'If you really want to fake that you\'ve lost your password, you should at the very least log out of Vipsu, you know.';
		} else {
	...
	*/
	public static function MaintenanceAlert() {
		try {
			...
			echo '<div class="alert alert-warning margin" role="alert"><p align="center"><i class="fa fa-cog fa-spin"></i>	Vipsu\'s website is in <b>maintenance mode</b>. Only moderators and administrators have access to the full website.</p></div>';
		}
		catch(Exception $e) {
			// Normal user, show alert and die
			echo '<div class="alert alert-warning margin" role="alert"><p align="center"><i class="fa fa-cog fa-spin"></i>	Vipsu\'s website is in <b>maintenance mode</b>. We are working for you, <b>please come back later.</b></p></div>';
			die();
		}
	}
	...
	*/
	public static function GameMaintenanceAlert() {
		try {
			...
			// Mod/admin, show alert and continue
			echo '<div class="alert alert-danger" role="alert"><p align="center"><i class="fa fa-cog fa-spin"></i>	Vipsu\'s score system is in <b>maintenance mode</b>. <u>Your scores won\'t be saved until maintenance ends.</u><br><b>Make sure to disable game maintenance mode from the admin control panel as soon as possible!</b></p></div>';
		}
		catch(Exception $e) {
			// Normal user, show alert and die
			echo '<div class="alert alert-danger" role="alert"><p align="center"><i class="fa fa-cog fa-spin"></i>	Vipsu\'s score system is in <b>maintenance mode</b>. <u>Your scores won\'t be saved until maintenance ends.</u></b></p></div>';
		}
	}
	...
	*/
	public static function BanchoMaintenanceAlert() {
		try {
			...
			// Mod/admin, show alert and continue
			echo '<div class="alert alert-danger" role="alert"><p align="center"><i class="fa fa-server"></i>	Vipsu\'s Bancho server is in maintenance mode. You can\'t play on Vipsu right now. Try again later.<br><b>Make sure to disable game maintenance mode from the admin control panel as soon as possible!</b></p></div>';
		}
		catch(Exception $e) {
			// Normal user, show alert and die
			echo '<div class="alert alert-danger" role="alert"><p align="center"><i class="fa fa-server"></i>	Vipsu\'s Bancho server is in maintenance mode. You can\'t play on Vipsu right now. Try again later.</p></div>';
		}
	}
			...
			foreach ($friends as $friend) {
				...
				<div class="ui '.($mutual ? 'mutual' : '').' column">
							<h4 class="ui image header">
								<img src="https://a.vipsu.ml/'.$friend['user2'].'" class="ui mini rounded image">
								<div class="content">
	...
	public static function AdminRankRequests() {
		...
		foreach ($rankRequests as $req) {
			...
			echo "<tr class='$rowClass'>
				<td><a href='https://vipsu.ml/s/$bsid' target='_blank'>$req[type]/$req[bid]</a></td>
				<td>$song</td>
				<td>
					$diffs
				</td>
	...
	*/
	...
	public static function AdminCharts() {
		...
		echo '</tbody>';
		echo '</table>';
        echo '<div class="text-center"><a href="/p/129"><button form="edit-badge-form" class="btn btn-primary">Add new beatmap</button></a><br></div>';
		echo '</div>';
		echo '</div></div></div>';
	}
