	...
	public static function tohtml($text, $advanced = false, $charset = 'utf-8') {
		...
		*/
		...
		if ($advanced) {
			 ...
			 * @var array
			*/
			$advanced_bbcode = ['#\[color=([a-zA-Z]*|\#?[0-9a-fA-F]{6})](.+)\[/color\]#Usi', '#\[size=([0-9][0-9]?)](.+)\[/size\]#Usi', '#\[quote](\r\n)?(.+?)\[/quote]#si', '#\[quote=(.*?)](\r\n)?(.+?)\[/quote]#si', '#\[url](.+)\[/url]#Usi', '#\[url=(.+)](.+)\[/url\]#Usi', '#\[email]([\w\.\-]+@[a-zA-Z0-9\-]+\.?[a-zA-Z0-9\-]*\.\w{1,4})\[/email]#Usi', '#\[email=([\w\.\-]+@[a-zA-Z0-9\-]+\.?[a-zA-Z0-9\-]*\.\w{1,4})](.+)\[/email]#Usi', '#\[img](.+)\[/img]#Usi', '#\[img=(.+)](.+)\[/img]#Usi', '#\[code](\r\n)?(.+?)(\r\n)?\[/code]#si', '#\[youtube]http://[a-z]{0,3}.youtube.com/watch\?v=([a-zA-Z0-9_-]{1,11})\[/youtube]#Usi', '#\[youtube]([a-zA-Z0-9_-]{1,11})\[/youtube]#Usi', '#\[container width=(.+) compact=1 center=(.+)](.*?)[/container]#Usi'];
			/*
			 * This array contains the advanced static bbcode's html.
			 *
			 * @var array
			*/
			$advanced_html = ['<span style="color: $1">$2</span>', '<font size=$1>$2</font>', "<div class=\"quote\"><span class=\"quoteby\">Disse:</span>\r\n$2</div>", "<div class=\"quote\"><span class=\"quoteby\">Disse <b>$1</b>:</span>\r\n$3</div>", '<a rel="nofollow" target="_blank" href="$1">$1</a>', '<a rel="nofollow" target="_blank" href="$1">$2</a>', '<a href="mailto: $1">$1</a>', '<a href="mailto: $1">$2</a>', '<img src="$1" />', '<img src="$2" width="'.explode("x","$1")[0].'" height="'.explode("x","$1")[1].'" />', '<div class="code">$2</div>', '<iframe width="560" height="315" src="https://www.youtube.com/embed/$1" frameborder="0" allowfullscreen></iframe>', '<iframe width="560" height="315" src="https://www.youtube.com/embed/$1" frameborder="0" allowfullscreen></iframe>', '<div style="width:'."$1".'px; margin: 0 auto;" class="compact-container "> $3 </div>'];
