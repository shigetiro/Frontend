...
class RequestRankedBeatmap {
	...
	const Title = 'Vipsu - Request Beatmap Ranking';
	const LoggedIn = true;
	...
	public function P() {
		...
		<div id="content">
			<div align="center">
				...
				<h4>Here you can send a request to rank an unranked beatmap on Gatari.</h4>';
	...
	public function DoGetData() {
		...
		try {
			...
			$matches = [];
			if (!preg_match("/https?:\\/\\/(?:osu|new)\\.(?:gatari|ppy)\\.(?:pw|sh)\\/(s|b)\\/(\\d+)/i", $_POST["url"], $matches)) {
				throw new Exception("Beatmap URL is not an osu.ppy.sh or new.ppy.sh URL.");
			}
            if ($matches[1] == "b"){
                $bm = $matches[2];
                $k = "ad10649c1e71221de38c3a6d2245b1276e7c38a5";
                $set = getJsonCurl("https://osu.ppy.sh/api/get_beatmaps?b=$bm&k=$k")[0]["beatmapset_id"];
            }else $set = $matches[2];
            $data = [
                "sid" => $set,
                "refresh" => 0
            ];
          $xd = postJsonCurl("http://new.vipsu.ml/letsapi/v1/cacheBeatmap",$data);
