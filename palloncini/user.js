...
function getFavourites(){
	...
	$.getJSON("/api/v1/users/favourites/" + UserID, {
		l: 8,
		p: currentPage["fav"],
	}, function(data) {
		if (data.code != 200) {
		...
		$.each(data.scores, function(k, v) {
			...
			u +=  '<td class="' + sw + '">\
						<p class="text-left">\
							...
							<a href="https://osu.gatari.pw/b/'+ v.beatmap.beatmap_id+'">'+ v.beatmap.song_name+ '</a>\
							<b>' + getScoreMods(v.mods) + '</b> (' + v.accuracy.toFixed(2) + '%) </p>\
							<div style=" margin: -4px 0px -4px 0px;">\
							<small><b>'+addCommas(v.score.toFixed())+' / '+v.max_combo+'x'+(v.fc == 0 ? '' : '<span style="color: '+(typeof MyUserID != "undefined" ? '#455' : 'rgb(220, 220, 220)')+';">('+v.fc+'x)</span>')+'</b>\
			...
			if (Mode == 0 || Mode == 3) {
                if(v.pp > 0)
                    ...
                    u += "<span title='" + addCommas(v.pp)+ "'>" + addCommas(v.score.toFixed()) + "</span>";
				if (type == "best") {
...
var d = new Date;
    var seconds =  Math.floor((new Date() - (date - 10800) ) / 1000 + d.getTimezoneOffset() * 60) ;
